export function initFixtureGenerator() {
  if (window._fixtureGeneratorInit) return;
  window._fixtureGeneratorInit = true;
...
/* ─── CLOUD FIRESTORE INTEGRATION (vrgc-main) ─── */
async function saveToFirestoreCloud() {
  ...
  try {
    const isBracket = FG.gameType === 'bracket';
    const payload = {
      name: isBracket
        ? (document.getElementById('bracket-name')?.value || 'Untitled Tournament')
        : (document.getElementById('pts-name')?.value || 'Untitled Tournament'),
      gameTitle: isBracket
        ? (document.getElementById('bracket-game-title')?.value || 'Valorant / CS2')
        : (document.getElementById('pts-game-title')?.value || 'BGMI / Free Fire'),
      gameType: FG.gameType,
      bracketType: FG.bracketType,
      bestOf: FG.bestOf,
      prizePool: isBracket
        ? (document.getElementById('bracket-prize')?.value || '')
        : (document.getElementById('pts-prize')?.value || ''),
      organizer: isBracket
        ? (document.getElementById('bracket-organizer')?.value || 'VRGC Events Team')
        : (document.getElementById('pts-organizer')?.value || 'VRGC Events Team'),
      venue: isBracket
        ? (document.querySelector('#venue-toggle .active')?.innerText.includes('LAN') ? 'lan' : 'online')
        : (document.querySelector('#pts-venue-toggle .active')?.innerText.includes('LAN') ? 'lan' : 'online'),
      teams: FG.teams,
      rounds: FG.bracketRounds,
      pointsTable: FG.pointsResults,
      placementPoints: FG.placementPoints,
      killPts: FG.killPts,
      killCap: FG.killCap,
      wwcdOn: FG.wwcdOn,
      wwcdBonus: FG.wwcdBonus,
    };
    ...
    alert('Could not save fixture to cloud database: ' + (err.message || err));
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.updateByesPill = updateByesPill;
  window.nextPowerOf2 = nextPowerOf2;
  window.updateKillCapHint = updateKillCapHint;
  window.updateMatchesHint = updateMatchesHint;
  window.renderPlacementTable = renderPlacementTable;
  window.loadPreset = loadPreset;
  window.addPlacementRow = addPlacementRow;
  window.removeThisRow = removeThisRow;
  window.removeLastPlacementRow = removeLastPlacementRow;
  window.resetToPreset = resetToPreset;
  window.readPlacementPointsFromTable = readPlacementPointsFromTable;
  window.addTeam = addTeam;
  window.removeTeam = removeTeam;
  window.renderTeamList = renderTeamList;
  window.parsePasteList = parsePasteList;
  window.updateTeamCounter = updateTeamCounter;
  window.updateGenerateBtn = updateGenerateBtn;
  window.switchTeamTab = switchTeamTab;
  window.generateFixture = generateFixture;
  window.generateSingleElim = generateSingleElim;
  window.generateDoubleElim = generateDoubleElim;
  window.generateRoundRobin = generateRoundRobin;
  window.chunkRRRounds = chunkRRRounds;
  window.generatePointsTable = generatePointsTable;
  window.renderStep4 = renderStep4;
  window.renderBracket = renderBracket;
  window.makeTeamSlot = makeTeamSlot;
  window.getRoundLabel = getRoundLabel;
  window.drawBracketConnectors = drawBracketConnectors;
  window.drawLine = drawLine;
  window.pickWinner = pickWinner;
  window.renderRoundRobin = renderRoundRobin;
  window.setRRWinner = setRRWinner;
  window.updateRRStandings = updateRRStandings;
  window.renderPointsTable = renderPointsTable;
  window.calcMatchPoints = calcMatchPoints;
  window.getSquadTotals = getSquadTotals;
  window.renderStandings = renderStandings;
  window.renderMatchEntryGrid = renderMatchEntryGrid;
  window.updateMatchResult = updateMatchResult;
  window.getGameTitle = getGameTitle;
  window.getMapRotation = getMapRotation;
  window.resetAndGoBack = resetAndGoBack;
  window.openPosterModal = openPosterModal;
  window.closePosterModal = closePosterModal;
  window.selectPosterTemplate = selectPosterTemplate;
  window.renderPosterCanvas = renderPosterCanvas;
  window.drawPoster = drawPoster;
  window.drawPosterBracket = drawPosterBracket;
  window.drawPosterMatchSlot = drawPosterMatchSlot;
  window.drawSlotTeamText = drawSlotTeamText;
  window.drawPosterPointsTable = drawPosterPointsTable;
